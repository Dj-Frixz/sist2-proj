...
def prisoners():
    return( pl
        .read_csv(csv('estat_crim_pris_ctz.tsv.gz'), separator='\t', null_values=["", ":", ": "])
        .select(
            pl.col('freq,citizen,unit,geo\\TIME_PERIOD')
            .str.split(',')
            .list.to_struct(fields=['freq','citizen','unit','state'])
            .alias("to_unnest"),
            pl.col('*').exclude('freq,citizen,unit,geo\\TIME_PERIOD','freq')
        )
        .unnest('to_unnest')
        .unpivot(
            index=['freq','citizen','unit','state'],
            value_name='number_of_prisoners',
            variable_name='year'
        )
        .with_columns(
            year = pl.col('year')
                    .str.replace(' ','')
                    .cast(pl.Int16),
            number_of_prisoners = pl.col('number_of_prisoners')
                                    .str.replace(' ', '')
                                    .cast(pl.Float64)
        )
        .pivot(
            on='citizen',
            values='number_of_prisoners'
        )
        .select(
            pl.col('*').exclude('FOR', 'NAT', 'TOTAL'),
            for_nat_ratio = pl.col('FOR') / pl.col('NAT')
        )
    )

def emigration():
    return( pl
        .read_csv(csv('estat_migr_emi2_filtered_en.csv'), separator=',', null_values="")
        .select(
            pl.col()
        )
    )
...
def life_exp():
    return(pl
           .read_csv(csv('estat_demo_mlexpecedu_filtered_en.csv'), separator=',', null_values='')
           .select(
               pl.col(['isced11','geo','TIME_PERIOD','OBS_VALUE'])
           )
           .pivot(
               on='isced11',
               values='OBS_VALUE'
           ...
           .rename(
               {'geo':'state',
                'TIME_PERIOD':'year',
                'Less than primary, primary and lower secondary education (levels 0-2)':'edulvl_0-2',
                'Upper secondary and post-secondary non-tertiary education (levels 3 and 4)':'edulvl_3-4',
                'Tertiary education (levels 5-8)':'edulvl_5-8',
                'All ISCED 2011 levels':'general_life_exp'}
           )
    )

def GDP():
    return(pl
           .read_csv(csv('estat_nama_10_pc_filtered_en.csv'), separator=',', null_values='')
           .select(
               pl.col(['geo','TIME_PERIOD','OBS_VALUE'])
           )
           .rename(
               {'geo':'state',
                'TIME_PERIOD':'year',
...
def life_exp_VS_GDP(year):
    return(
        life_exp().join(GDP(), on=['state','year'], how='inner')
...
def immigration():
    return(
        pl
        .read_csv(csv('estat_tps00176_filtered_en.csv'), separator=',', null_values='')
        .select(
            pl.col(['geo','TIME_PERIOD','OBS_VALUE'])
        )
        .rename(
            {'geo':'state',
             'TIME_PERIOD':'year',
             'OBS_VALUE':'immigrants'}
        )
    )

def immigration_vs_gdp():
    return(
        immigration().join(GDP(), on=['state','year'], how='inner')
        .with_columns(
...
def pm25():
    return(
        pl.read_csv(csv('estat_sdg_11_52_filtered_en.csv'), separator=',', null_values='')
        .select(
            pl.col(['geo','unit','OBS_VALUE']),
            pl.col('TIME_PERIOD').cast(pl.Int16)
        )
        .rename(
            {'geo':'state',
             'TIME_PERIOD':'year',
             'OBS_VALUE':'pm2.5_deaths'}
        )
    )

def population():
    return(
        pl.read_csv(csv('estat_cens_21dwbno_r3_filtered_en.csv'), separator=',', null_values='')
        .select(
            pl.col(['geo','OBS_VALUE'])
        )
        .rename(
            {'geo':'state',
...
def continents():
    return(
        pl.read_csv(csv('country-and-continent-codes-list-csv.csv'), separator=',', null_values='')
        .rename({'Three_Letter_Country_Code':'Code'})
        .select(pl.col(['Continent_Name','Continent_Code','Code']))
    )
